...
private const static string EXP_AI_FOLDER = EXPANSION_FOLDER + "AI\\"; // root directory for PatrolSettings
...
void InitDynamicPatrols()
{
    ...
    if ( !FileExist(EXP_AI_SETTINGS) )
    {
		...
		Print("Patrol config will be located in: "+EXP_AI_SETTINGS);
    ...
    if (m_ExpAIGroups.RespawnTime == -1)
        Print("Bots respawn is disabled (RespawnTime is set to -1)");

    if (m_ExpAIGroups.MinDistRadius > m_ExpAIGroups.MaxDistRadius)
    {
        Print("!!! ERROR !!!");
        Print("MinDistRadius has a larger radius than MaxDistRadius (MinDistRadius should be smaller than MaxDistRadius)");
        Print("!!! ERROR !!!");
    }
    ...
    foreach(ExpAIGroup group: m_ExpAIGroups.Group)
    {
		...
		if ( group.NumberOfAI != 0 )
		{
            ...
            Print("WARNING: NumberOfAI shouldn't be set to 0, skipping this group...");
			continue;
		}
        ...
        {
            Print("!!! ERROR !!!");
            Print("Couldn't read the Waypoints (validate your file with a json validator)");
            Print("!!! ERROR !!!");
            continue;
        }
		...
		if ( !startpos || startpos == "0 0 0" )
		{
            ...
            {
                Print("!!! ERROR !!!");
                Print("Couldn't find a spawn location. StartPos and at least the first Waypoints are both set to 0 0 0 or cannot be read by the system (validate your file with a json validator)");
                Print("!!! ERROR !!!");
                continue;
            }
		...
		Print("Spawning "+aiSum+" "+group.Faction+" bots at "+group.StartPos+" and will patrol at "+group.Waypoints);
 		eAIDynamicPatrol.Create(startpos, group.Waypoints, ExpInit_GetAIBehaviour(group.Behaviour), group.LoadoutFile, aiSum, m_ExpAIGroups.RespawnTime, ExpInit_GetAIFaction(group.Faction), true, m_ExpAIGroups.MinDistRadius, m_ExpAIGroups.MaxDistRadius, ExpInit_GetAISpeed(group.Speed));
	}
	Print("=================== Expansion AI ===================");
}
...
void ExpInit_DefaultPatrolsSettings(out ExpAIGroups Data)
{
    ...
    switch (worldName)
    {
        ...
        case "enoch":	//Tested Good Points and Loadouts. Waypoint Behavior specific to the waypoints, not all behaviors will work correctly with some routes
            Data.Group.Insert( new ExpAIGroup(10, 	"WALK",    	"REVERSE",	"INSURGENT",    "SoldierCDFLoadout.json",  	"6453.31 174.932 10981.6",      		{"6453.31 174.932 10981.6", "6534.43 174.578 11023.00", "6615.63 173.571 11067.5", "6687.14 174.054 11103.7"} ));
            Data.Group.Insert( new ExpAIGroup(10, 	"WALK",    	"REVERSE", 	"CIVILIAN",     "SoldierMOTRSLoadout.json", "6303.47 381.472 3911.79",      		{"6303.47 381.472 3911.79", "6317.71 379.912 3870.99", "6330.94 380.986 3840.62", "6337.35 381.841 3819.01", "6344.2 382.049 3788.23", "6397.86 382.435 3715.53", "6413.77 381.767 3698.73", "6447.35 381.685 3674.81", "6474.48 381.403 3664.66", "6513.5 382.929 3663.82", "6584.13 379.569 3557.25", "6637.13 366.449 3490.86", "6652.16 357.021 3435.77", "6681.99 346.724 3395.99", "6712.61 328.543 3335.31", "6727.83 322.986 3324.47", "6744.66 319.581 3326.5", "6767.47 315.498 3338.96", "6792.34 311.333 3342.37", "6826.68 303.954 3331.26", "6867.14 299.576 3324.07", "6907.82 299.158 3298.6", "6938.85 298.278 3285.71", "6976.62 300.97 3247.94", "6987.2 300.871 3203.35", "7008.57 299.646 3181.55", "7095.26 296.592 3144.57", "7187.00 295.347 3085.46"} ));
            Data.Group.Insert( new ExpAIGroup(10, 	"WALK",	 	"REVERSE", 	"EAST",     	"SoldierMOTRSLoadout.json", "9272.91 199.589 10826.2",  			{"9272.91 199.589 10826.2", "9266.68 199.532 10876.1", "9270.63 199.459 10937.9", "9250.82 198.64 10983.2", "9164.87 198.142 11063.4"} ));
            Data.Group.Insert( new ExpAIGroup(10, 	"WALK",    	"REVERSE", 	"EAST",			"Raiderloadout.json",    	"9486.09 242.261 10360.6",      		{"9486.09 242.261 10360.6", "9435.31 242.442 10421.8", "9394.87 238.669 10507.00", "9382.44 232.412 10558.3", "9371.1 217.304 10683.0"} ));
        break;
        case "esseker":
            Data.Group.Insert( new ExpAIGroup(5, 	"WALK",     "LOOP",     "WEST",      	"SoldierCDFLoadout.json",  	"6137.18 8.1828 14667.3",               {"6137.18 8.1828 14667.3", "6127.65 8.1598 14655.7", "6127.19 6.34518 14647.1"} ));
            Data.Group.Insert( new ExpAIGroup(5, 	"WALK",     "LOOP",     "EAST",      	"SoldierMOTRSLoadout.json", "6167.18 8.1828 14667.3",               {"6167.18 8.1828 14667.3", "6127.65 8.1598 14655.7", "6127.19 6.34518 14647.1"} ));
	...
	Print("Generating an example PatrolSettings.json for: "+worldName);
}
